    ...
    if check_cuda; then
        ...
        if command -v nvcc &> /dev/null; then
            ...
            if [[ "$nvcc_major" -gt 12 || ("$nvcc_major" -eq 12 && "$nvcc_minor" -ge 1) ]]; then
                print_step "CUDA $nvcc_version is already installed and meets requirements (>=12.6)"
                export CUDA_HOME=${CUDA_HOME:-$(dirname $(dirname $(which nvcc)))}
            else
                print_step "CUDA version < 12.6, installing CUDA toolkit 12.6..."
                conda install -c "nvidia/label/cuda-12.6.0" cuda-toolkit -y
                export CUDA_HOME=$CONDA_PREFIX
            fi
        else
            print_step "NVCC not found, installing CUDA toolkit 12.6..."
            conda install -c "nvidia/label/cuda-12.6.0" cuda-toolkit -y
            export CUDA_HOME=$CONDA_PREFIX
        fi
